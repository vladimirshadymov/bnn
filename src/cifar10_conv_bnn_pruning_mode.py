...
class Cifar10ConvBNN(nn.Module):
    def __init__(self, stochastic=False):
        ...
        self.layer512_1 = nn.Sequential(
            nn.ReplicationPad2d(1),
            BinarizedConv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, p=self.pruning),
            nn.BatchNorm2d(512),
            # nn.Dropout2d(self.dp),
            Binarization(self.stochastic_mode)
        )
        ...
        self.layer512_2 = nn.Sequential(
            nn.ReplicationPad2d(1),
            BinarizedConv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, p=self.pruning),
            nn.MaxPool2d(2),
            nn.BatchNorm2d(512),
            # nn.Dropout2d(self.dp),
            Binarization(self.stochastic_mode)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), "../model/cifar10_conv_bnn_pruning_mode.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open('../model/cifar10_conv_bnn_pruning_mode_report.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
