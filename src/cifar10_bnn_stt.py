...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train(args, model, device, train_loader, optimizer, epoch, penalty='multi_margin')
        test(args, model, device, test_loader, train_loader, test_accuracy, train_accuracy, penalty='multi_margin')
        scheduler.step(epoch=epoch)
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), "../model/cifar10_bnn_stt.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open('../model/cifar10_bnn_stt_report.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
