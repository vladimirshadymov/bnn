...
from bnn_modules import PruningConv2d
import argparse
import torch
...
class Cifar10CNN(nn.Module):
    def __init__(self):
        ...
        self.conv3 = PruningConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1, p=self.pruning)
        self.bn3 = nn.BatchNorm2d(256)

        self.conv4 = PruningConv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1, p=self.pruning)
        self.bn4 = nn.BatchNorm2d(256)

        self.conv5 = PruningConv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1, p=self.pruning)
        self.bn5 = nn.BatchNorm2d(512)

        self.conv6 = PruningConv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1, p=self.pruning)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), "../model/cifar10_cnn_pruning_mode.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open('../model/cifar10_cnn_report_pruning_mode.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
